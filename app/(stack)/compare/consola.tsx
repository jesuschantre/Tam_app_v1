...
import { getConsolas } from '../../../services/productService'

// Criterios de rendimiento para consolas
const puntajeConsola = (p: MockProduct): number => {
  let puntos = 0
  ...
  // Procesador/Rendimiento (PRIORIDAD MÁS ALTA)
  if (todo.includes('amd zen 2')) puntos += 40
  else if (todo.includes('custom')) puntos += 30

  // Almacenamiento (IMPORTANTE)
  if (todo.includes('1tb')) puntos += 35
  else if (todo.includes('825gb')) puntos += 20

  // Resolución
  if (todo.includes('4k')) puntos += 25
  else if (todo.includes('1440p')) puntos += 15

  // Conectividad
  if (todo.includes('wifi 6')) puntos += 12
  else if (todo.includes('wifi 5')) puntos += 8

  // Batería
  if (todo.includes('5220')) puntos += 10
  else if (todo.includes('5000')) puntos += 8

  return puntos
}

const ConsolaCompareScreen = () => {
  const [consolas, setConsolas] = useState<MockProduct[]>([])
  ...
  const filtered = consolas.filter(p =>
  ...
  const puntos1 = product1 ? puntajeConsola(product1) : 0
  const puntos2 = product2 ? puntajeConsola(product2) : 0
      ...
      <View>
        ...
        >
          <View style={{
            ...
            <TextInput
              placeholder="Buscar consola para comparar..."
              ...
              {/* TABLITAS */}
                ...
                const esMejor = (titulo: string, valor1: string, valor2: string): 1 | 2 | 0 => {
                    ...
                    const extractSize = (val: string) => {
                        if (val.includes('1tb')) return 1024
                        if (val.includes('825gb')) return 825
                        return 0
                    }
                    ...
                    if (titulo.toLowerCase().includes('resolución') || titulo.toLowerCase().includes('4k')) {
                    if (valor1.includes('4k') && !valor2.includes('4k')) return 1
                    if (valor2.includes('4k') && !valor1.includes('4k')) return 2
                    }

                    if (titulo.toLowerCase().includes('wifi') || titulo.toLowerCase().includes('conectividad')) {
                    const orden = ['wifi 5', 'wifi 6']
                    ...
                    if (valor1.toLowerCase().includes('zen 2') && !valor2.toLowerCase().includes('zen 2')) return 1
                    if (valor2.toLowerCase().includes('zen 2') && !valor1.toLowerCase().includes('zen 2')) return 2
                    }
...
export default ConsolaCompareScreen